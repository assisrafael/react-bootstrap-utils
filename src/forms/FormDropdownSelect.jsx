...
export function FormDropdownSelect({
  ...
  // const items = options; // verificar
  const _selectedItem = options.find((item) => item.value === value);
  ...
  const onSelectionInputFocus = useCallback(() => {
    if (openOnFocus) {
      setTimeout(() => {
        open();
      }, 100);
  ...
  // const onSelectionInputBlur = useCallback(() => {
    ...
    <>
      <input
        ...
        className={formatClasses([
          'form-control form-autocomplete-search h-auto',
          isFocused ? '' : 'd-none',
          controlFeedback,
        ])}
        // onChange={inputHandleChange}
        onFocus={onSelectionInputFocus}
        // onBlur={onSelectionInputBlur}
        value={placeholder}
        // value={searchValue}
        role="combobox"
      ...
      {!isFocused && (
        <div
          className={formatClasses(['form-control form-autocomplete-selected h-auto', controlFeedback])}
          disabled={disabled}
...
FormDropdownSelect.defaultProps = {
  openOnFocus: false,
  // onSearch: () => {},
...
export function FormGroupDropdownSelect(props) {
  return (
    <FormGroup {...props}>
      <FormDropdownSelect {...props} />
    </FormGroup>
  );
}

FormGroupDropdownSelect.propTypes = {
  afterChange: PropTypes.func,
  // allowUnlistedValue: PropTypes.bool,
  disabled: PropTypes.oneOfType([PropTypes.bool, PropTypes.func]),
  // filter: PropTypes.func,
